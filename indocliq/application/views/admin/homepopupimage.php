...
<div class="content-wrapper">
  ...
  <section class="content-header"><h1><img src="<?php echo base_url().'common_assets/images/categories.png';?>"> Pop up Image<small></small></h1></section>
  <!-- Main content -->
      ...
      <div class="col-xs-12">
        <div class="box">
          <!-- <div class="box-header col-md-6" style="float: left;">
            <h3 class="box-title">
                <a href="<?php echo base_url();?>admin/addhomepopupimage" class="btn btn-primary pull-right">Add New pop up Image</a>
            </h3>
          </div> -->
          ...
          <div class="box-body table-responsive">
            <table id="categorytable" class="table table-bordered table-hover">
              ...
              <tbody>
                ...
                <?php $count = 1; 
                  if(!empty($popup_data)){
                    foreach ($popup_data as $getdata) { 
                  ?>
                    <tr>
                      <td><?php echo $count; ?></td>
                      <!--<td><?php  echo (!empty($getdata['title'])?$getdata['title']:'none'); ?></td>-->
                      <td><?php  echo (!empty($getdata['image'])?'<img src='.base_url().'uploads/homepopupimage/'.$getdata['image'].' style="width:50px;height:50px">':'none'); ?></td>
                      ...
                      <td>
                        <a href="<?php echo base_url() ?>admin/addhomepopupimage/<?php echo  $getdata['id']; ?>" title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a> 
  ...
  $(".delete").click(function(e){
    ...
    if(val){
      $.ajax({
        ...
        data:{tablename:'homepopupimage',id:id,status:3,whrcol:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        ...
        if(val){
            $.ajax({
              type: "POST",
              url: "<?php echo base_url();?>admin/change_status",
              data:{tablename:'homepopupimage',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".activecat").click(function(e){
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>admin/change_status",
        data:{tablename:'homepopupimage',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
