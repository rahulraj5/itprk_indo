...
class Shop extends CI_Controller {
	function __construct()
	{
		...
		$this->SessionModel->checkshoplogin(array("login","loginajax","registerajax","loginbyotpsubmit","verifyloginOtp","createCode"));
	}
    ...
    public function login_otp_send($mobile_no,$country_isd_code){
		$mobile_decode_no = base64_decode($mobile_no);
		if($mobile_no){
	...
	public function loginbyotpsubmit(){
		$mobile_no = base64_encode(trim($_POST['moibleno']));
		$checkmobileno = $this->Common_model->getwhere('shops', array('mobile_no'=>$mobile_no));
		// dd($checkmobileno);
...
// 		print_r($data['categorylist']);
    ...
    public function support_ticket(){
        ...
        $data = array();
        $data['support'] = $this->Common_model->getWhereData('support_ticket',array('user_type'=>'shop','user_id'=>$user_id));
