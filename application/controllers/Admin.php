...
class Admin extends CI_Controller {
	...
	public function sub_subcategories()
	{
		...
		// $whr = array();
		// $whr[] = "c2.categories_id = c1.parent_category_id";
		// $whr[] = array('status !='=>3,'parent_category_id !='=>0,'parent_category_id !='=>1);
		// $whr2 = " WHERE ".implode(" AND ", $whr);
...
// 		$data['seller_data'] = $this->Common_model->GetWhere('seller_agreement',$data);
	...
	public function cancel_return_policy($id = "")
	{
        $data = array();
		if(isset($_POST) && !empty($_POST)){
			$insert_data = array();
			...
			if (isset($_POST['id']) &&  !empty($_POST['id'])){
				$result_id = $this->Common_model->updateRecords('cancel_return',$insert_data,array('id'=>$_POST['id']));
			}else{
				$result_id = $this->Common_model->addRecords('cancel_return',$insert_data);
			}
		}
	
		$whr = array('id'=>2);
		$data['cancel_return_data'] = $this->Common_model->getSingleRecordById('cancel_return', $whr);
		
		$this->load->view('admin/header');
		$this->load->view('admin/cancel_return_policy',$data);
		$this->load->view('admin/footer');
	}
	...
	public function faqs($id = "")
	{
		...
		$whr = array('id'=>2);
		$data['faqs_data'] = $this->Common_model->getSingleRecordById('faqs_data', $whr);
